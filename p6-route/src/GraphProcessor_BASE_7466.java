...
public class GraphProcessor {
    ...
    public double routeDistance(List<Point> route) {
        ...
        for (int i = 1; i< route.size(); i++){
            distance += first.distance(route.get(1));
            if (i < route.size()-1){
                first = route.get(i);
            }
    ...
    public List<Point> route(Point start, Point end) throws InvalidAlgorithmParameterException {
        ...
        HashSet<Point> aList = map.get(current);

        while (!toExplore.isEmpty()){
            current = toExplore.remove();
            for (Point neighbor: aList){
        ...
        /*for (Point keys: previous.keySet()){
            Point cur = start;
            if (previous.get(keys).equals(cur)){
                ret.add(keys);
                cur = keys;
            }
        }*/
